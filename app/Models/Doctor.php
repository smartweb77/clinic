...
class Doctor extends Model
{
    ...
    private static $translates_table = 'doctor_translates';
    ...
    public static function getItemInfo($id = 0, $local = '')
    {
        if (property_exists(__CLASS__, 'translates_table')) {
            return self::$current_class::join(self::$translates_table, self::$main_table.'.id', '=', self::$translates_table.'.parent_id')
                ...
                ->select(
                    self::$main_table.'.*',
                    self::$translates_table.'.full_name',
                    self::$translates_table.'.specialty',
                    self::$translates_table.'.education',
                    self::$translates_table.'.experience'
                )
                ->first();
        } else {
        ...
        if (property_exists(__CLASS__, 'translates_table')) {
            return self::$current_class::join(self::$translates_table, self::$main_table.'.id', '=', self::$translates_table.'.parent_id')
                ...
                ->when($search, function ($query, $search) {
                    return $query->where(self::$translates_table.'.full_name', 'LIKE', "%{$search}%");
                })
                ->orderBy('sort', 'asc')
